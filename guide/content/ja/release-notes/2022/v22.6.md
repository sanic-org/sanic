...
This is the second release of the version 22 [release cycle](../../organization/policies.md#release-schedule). バージョン22は、12月の長期サポートバージョンリリースで「確定」されます。
...
HTTP/3 は TLS を必要とするため、TLS 証明書なしでは HTTP/3 サーバーを起動できません。 [set up yourself](../../guide/how-to/tls.md) または `DEBUG`モードの場合は`mkcert`を使用してください。 現在、HTTP/3 に対する自動的な TLS 設定は `trustme` と互換性がありません。
