...
The `Route` object in v21.3 no longer had a `uri` attribute. Instead, the closes you could get was `route.path`. However, because of how `sanic-routing` works, the `path` property does _not_ have a leading `/`. This has been corrected so that now there is a `route.uri` with a leading slash:
...
You can now pass your own config and context objects to your Sanic applications. A custom configuration _should_ be a subclass of `sanic.config.Config`. The context object can be anything you want, with no restrictions whatsoever.
...
Now, you can alter the prefix (and therefore add path segments _before_ the version).
