...
1. 你可以不用直接使用 :class:`sanic.response.HTTPresponse` 实例去返回数据 使用[方便方法](./response.md#methods)非常简单。

   - `from sanic import json`
   - `from sanic import html`
   - `from sanic import redirect`
   - _etc_
2. 正如我们将在[串流部分](../advanced/streaming.md#response-streaming)中看到的那样，您并不总是需要返回一个对象。 如果您使用这个底层的 API，您可以在处理程序内部控制响应的流程，并且不需要使用返回对象。
