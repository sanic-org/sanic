...
这是版本 22 的第二次版本[发行周期](../../organization/policies.md#release-schedule)。 版本22将在12月份的长期支持版本发布时“最后完成”。
...
因为HTTP 3 需要 TLS，您不能在没有TLS 证书的情况下启动 HTTP/3 服务器。 您应该[自己设置它](../../guide/how-to/tls.md) 或者在 `DEBUG` 模式下使用 `mkcert` 。 目前，HTTP/3 的自动TLS设置与 `trustme` 不兼容。
