    ...
    def generate_random_direct_route(max_route_depth=4):
        ...
        for depth in range(1, max_route_depth + 1):
            for _ in range(RouteGenerator.ROUTE_COUNT_PER_DEPTH):
                ...
                route_detail = (
                    random.choice(RouteGenerator.HTTP_METHODS),
                    route,
                )
...
async def test_resolve_route_no_arg_string_path():
    ...
    router, simple_routes = setup_router(
        route_details=simple_routes, app_name="benchmark_simple"
    )
    ...
    handler_result = await result.get(
        "_test_result",
        router.get(request.path, request.method, request.headers.get("host")),
    )[1](None)
...
async def test_resolve_route_with_typed_args():
    ...
    router, typed_routes = setup_router(
        route_details=typed_routes, app_name="benchmark_typed"
    )
...
async def main():
    ...
    try:
        await test_resolve_route_no_arg_string_path()
        await test_resolve_route_with_typed_args()
